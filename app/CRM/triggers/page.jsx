...
export default function EmailTriggersPage() {
  ...
  const fetchTriggers = async () => {
    try {
      const res = await fetch('/CRM/api/triggers');
  ...
  const handleAddTrigger = async () => {
    ...
    try {
      const res = await fetch('/CRM/api/triggers', { method: 'POST', body: JSON.stringify(newTrigger) });
      const data = await res.json();
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`/CRM/api/triggers?id=${id}`, { method: 'DELETE' });
  ...
  const saveEdit = async (id) => {
    try {
      const res = await fetch('/CRM/api/triggers', { method: 'PUT', body: JSON.stringify(editData) });
      const data = await res.json();
      if (data.trigger) {
