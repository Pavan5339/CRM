...
export default function RootLayout({ children }) {
  return (
    <html lang="en" className={`${spaceMono.variable} ${bricolageGrotesque.variable} ${dmSans.variable}`}>
      ...
      <body suppressHydrationWarning className="antialiased bg-(--bg) bg-[radial-gradient(circle_at_20%_50%,rgba(200,134,10,0.04)_0%,transparent_50%),radial-gradient(circle_at_80%_20%,rgba(42,114,195,0.04)_0%,transparent_50%)] text-(--text) font-(family-name:--body) min-h-screen overflow-x-hidden">
        {children}
      </body>
    </html>
  );
